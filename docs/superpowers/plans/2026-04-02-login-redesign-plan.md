...
**Goal:** Refondre la page `/login` avec un layout 60/40 split-screen, panneau blanc sans tabs, champs underline-only, identité visuelle cohérente avec le logo Kayvila.
...
## Task 2 : LoginForm — panel + header KAYVILA + footer
...
```bash
git add app/login/page.tsx
git commit -m "feat(login): KAYVILA text header, 26rem panel, footer links redesign"
```
...
# ✓ Panneau blanc à droite, "KAYVILA" en petites capitales
