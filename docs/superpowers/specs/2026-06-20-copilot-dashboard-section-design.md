Design solide. Mes remarques :

👍 Ce qui est bon :
- Chat intégré plutôt que FAB flottant — desktop-first, plus pro, mieux utilisé
- Split Server/Client propre sans toucher au Provider
- 3 actions concrètes, pas juste du blabla
- Vérification propriété systématique (anti-IDOR)
- tsc --noEmit avant push

⚠️ Points d'attention :

1. SET_PRICE — vérifie que la colonne villas utilise price_per_night (pas price_night ou juste price). Le schéma Kayvila c'est price_per_night.

2. SHOW_BOOKING — la query gte("start_date") ne couvre que les futurs check-ins. Si le proprio dit "qui est chez moi en ce moment", ça rate les séjours en cours. Ajoute une clause OR (start_date <= today AND end_date >= today).

3. Piège Server→Client — le DashboardPageClient wrappe tout, OK. Mais si un jour tu passes un callback ou une promesse en props depuis le Server Component parent, crash masqué en prod. Mets un commentaire dans le code pour prévenir.

4. ProactiveNotification — mentionné dans le layout mais pas détaillé. C'est le digest matinal de l'agent B ? Si oui, faut le connecter à la spec qu'on a validée avec Claude.

Hormis ça, RAS. C'est rangé, c'est utile, c'est sécurisé# Spec — Copilot "Diamant" intégré dashboard + Actions
