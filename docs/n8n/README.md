...
Elle crée `conversation_memory` (historique conversationnel) et `banned_sessions` (sécurité).
...
> **Auth interne (B et C)** : aucune clé statique à remplacer. Chaque appel aux routes protégées
> forwarde le **JWT Supabase de l'utilisateur** (reçu dans `body.token`) en en-tête
> `Authorization: Bearer <token>`. Le serveur valide le token et dérive lui-même le périmètre
> (owner / admin) — on ne fait jamais confiance à un paramètre d'URL.
...
- **Sécurité** : `Check Banned Session` (Postgres) → `IF Session Bannie` (agent A) ; `Auth JWT` → `IF Auth Valide` (B/C).
- **Mémoire courte** : `Get Long-Term Memory` (20/30/50 msg) + `Save Memory` (Supabase) + `Postgres Chat Memory` (agent).
- **Mémoire longue sémantique** : `gbrain Recall` (avant LLM) + `gbrain Capture` (après LLM) — **B et C uniquement**.
