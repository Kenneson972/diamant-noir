# Automatisation n8n — Copilot propriétaire Kayvila
...
5. **Activer** le workflow, puis copier l’URL **Production** du Webhook (path `kayvila-owner-copilot`) → la mettre dans **`N8N_OWNER_WEBHOOK_URL`** sur Vercel / `.env.local`.
...
1. **Webhook** — méthode `POST`, path ex. `/webhook/kayvila-owner-copilot`, **Respond: Using Respond to Webhook Node**.
...
```
Tu es l’assistant conciergerie Kayvila pour UN propriétaire de villas de luxe en Martinique.
