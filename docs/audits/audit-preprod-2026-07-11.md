...
> **Mise à jour 2026-07-12 : P0 1-3 corrigés/traités** (commit dédié, voir détail ci-dessous). Le P0 4 reste une action manuelle Ken.

1. ✅ **CORRIGÉ — Contrainte anti-double-booking en base** (Critique 1) : migration `supabase/migrations/20260712150000_bookings_no_overlap.sql` (`btree_gist` + `EXCLUDE USING gist (villa_id WITH =, daterange(start_date, end_date) WITH &&) WHERE status IN ('pending','confirmed','paid')`), **appliquée au projet Supabase**. Vérifiée empiriquement en SQL : chevauchement rejeté (`exclusion_violation`), dates adjacentes acceptées (fin exclusive, alignée sur le check applicatif), statuts annulés exemptés. La route booking gère désormais le code `23P01` → 409 « villa non disponible » (`app/api/booking/route.ts`).
2. ✅ **CORRIGÉ — Chemin `/success?bookingId=` sécurisé** (Critique 2) : `/api/booking-session` accepte maintenant `bookingId` (UUID validé) et vérifie le statut réel en BDD (+ sync Stripe via la session liée au booking si présente, `stripe_session_id` jamais exposé dans la réponse). La page succès n'affiche plus jamais la confirmation sur la seule foi du paramètre : réservation non payée → écran distinct « Réservation en attente de paiement » (i18n fr/en/es) ; UUID forgé/inexistant → « Réservation introuvable ». Les deux comportements vérifiés en navigateur.
3. ✅ **TRAITÉ — Villa seedée + suites re-testées** (Critique 3) : villa « Villa Test Préprod » créée (id `fee07a75-2f3d-4a57-8219-5b49ace2f536`, **dépubliée après les tests** car la base est partagée — la republier + `TEST_VILLA_ID=<id>` pour re-tester). Résultat : **Playwright 40 passed / 1 skipped** (contre 28/13 à l'audit) — les 16 tests checkout jusque-là jamais exécutés passent (checkout CGV, redirection `/success`, idempotence double-clic, rate limiting, Connect 503). Dernier skip restant : « Aucune réservation remboursée en base » (`stripe-admin-refund`). Vitest : 202 tests passent (les 5 suites `lib/proactive` cassées = P1 n°5, inchangé). **Reste un vrai paiement test sur la page Stripe hébergée à dérouler une fois sur l'environnement de préprod déployé** (non couvert par les suites mockées).
4. 🔍 **Action Ken — Dashboard Stripe** (Critique 4) : configurer/vérifier l'endpoint webhook prod + les 12 événements + reporter le `whsec_` prod ; au moment de la bascule, remplacer `sk_test_` par les clés live dans l'env prod (jamais dans le repo).

### P1 — semaine de bascule

> **Mise à jour 2026-07-12 : les 4 P1 sont corrigés** (commit dédié, vitest 37/37 suites · 227 tests, vérification navigateur faite).

5. ✅ **CORRIGÉ — 5 suites vitest cassées** : ajout de la transformation JSX dans `vitest.config.ts` (`oxc.jsx.runtime: "automatic"` — le tsconfig Next reste en `jsx: preserve`). Les 5 suites `lib/proactive/*` chargent et passent : 37/37 suites, 227 tests (25 récupérés).
6. ✅ **CORRIGÉ — Villa inexistante → 404 propre** : `app/villas/[id]/page.tsx` — la `fallbackVilla` fictive est supprimée, villa introuvable/dépubliée/erreur → `notFound()` (page 404 brandée). Bonus découvert en corrigeant : le `notFound()` existant pour villa dépubliée était **avalé par le try/catch** (il n'a jamais fonctionné) — restructuré pour décider du 404 hors du catch. Les erreurs console `villa_id=fallback` (400/500) disparaissent de fait. Même famille : les 4 requêtes villa de l'espace client (`reservations/[id]`, `livret`, `livret/print`, `checklist`) + `send-booking-confirmation` passent de `.single()` à `.maybeSingle()` → plus de 406 quand la villa a été supprimée.
7. ✅ **CORRIGÉ — Incohérence Revenus proprio** : cause racine trouvée — la page `/dashboard/revenus` sélectionnait 3 colonnes **inexistantes** (`stripe_transfer_*`), la requête échouait silencieusement → 0 € partout. Colonnes retirées du select (mappées à `null`, la table les affiche conditionnellement), erreur de requête désormais loggée, cartes « ce mois » + graphique alignés sur le périmètre exact du KPI dashboard (mois courant par `start_date`, paiement `paid` ou legacy `null`). Vérifié en navigateur : les deux vues affichent **6 435 €**. Corrections annexes dans la table : montants en centimes formatés comme des euros (« 866.3K€ » → « 8 663 € », format exact sans abréviation), warning React key (Fragment), texte du graphique « Aucun revenu ce mois-ci » remplacé par « Pas encore assez d'historique… » (c'est ce qu'il signifie réellement).
8. ✅ **CORRIGÉ — Alerte admin sur échec d'email de confirmation** : le webhook vérifie désormais la réponse de `send-booking-confirmation` (HTTP non-2xx **ou** `emailSent:false` — avant, seule une erreur réseau était détectée) et insère une notification `type:"system"` dans la table `notifications` (visible dashboard admin, Realtime) : « Email de confirmation non envoyé … à renvoyer manuellement ». Pas de file de retry (chantier plus lourd), mais l'équipe est prévenue immédiatement.
