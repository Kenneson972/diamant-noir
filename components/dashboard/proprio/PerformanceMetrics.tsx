...
const metrics = [
  {
    icon: <KayvilaPngIcon name="star" size={20} className="text-gold" aria-hidden />,
    label: "Taux d'occupation",
    getValue: (p: PerformanceMetricsProps) => `${p.occupancyRate}%`,
  },
  {
    icon: <Moon className="h-5 w-5 text-gold" strokeWidth={1.5} aria-hidden />,
    ...
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      {metrics.map((metric) => (
        ...
        >
          ...
          <span className="mt-3 block text-sm font-medium text-navy/80">
            {metric.label}
          </span>
