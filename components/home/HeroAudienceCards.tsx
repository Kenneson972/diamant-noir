...
export function HeroAudienceCards() {
  ...
  const chooseProprio = useCallback(() => {
    ...
    notifyHomeAudienceChange();
    setSelection("proprietaire");
    const el = document.getElementById("offre-proprietaire");
    if (el) {
  ...
  /* Après choix propriétaire : les cartes disparaissent (scroll vers la section) */
    ...
    <div className="mx-auto mt-8 flex w-full max-w-2xl flex-col gap-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <button
          ...
          className="group flex min-h-[104px] flex-col items-start gap-2 border border-white/20 bg-black/20 px-6 py-6 text-left backdrop-blur-sm transition-all duration-200 hover:border-gold/40 hover:bg-black/28 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold/50 active:scale-[0.98]"
        >
          <span className="text-[9px] font-bold uppercase tracking-[0.38em] text-gold/75">
            Espace Voyageur
          </span>
          ...
          <Search
            className="mt-auto h-[15px] w-[15px] text-gold/55 transition-transform duration-200 group-hover:scale-110"
            strokeWidth={1.5}
            aria-hidden
          />
        </button>
        ...
        >
          ...
          <Building2
            className="mt-auto h-[15px] w-[15px] text-white/30 transition-transform duration-200 group-hover:translate-x-0.5"
            strokeWidth={1.5}
            aria-hidden
          />
        </button>
