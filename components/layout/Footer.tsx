...
export const Footer = () => {
  ...
  return (
    <footer className="border-t border-black/10 bg-white pt-12 text-navy md:pt-16" style={{ paddingBottom: 'calc(3rem + env(safe-area-inset-bottom, 0px))' }}>
      <div className="mx-auto max-w-7xl px-5 sm:px-6">
        ...
        <div className="md:hidden">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <h4 className="mb-4 text-[11px] font-bold uppercase tracking-[0.25em] text-black/35">{t("footer.explore")}</h4>
            ...
            <div>
              <h4 className="mb-4 text-[11px] font-bold uppercase tracking-[0.25em] text-black/35">{t("footer.support")}</h4>
              <ul className="space-y-2 text-sm text-navy/65">
          ...
          <div className="mt-8 border-t border-black/10 pt-6">
            <h4 className="mb-4 text-center text-[11px] font-bold uppercase tracking-[0.25em] text-black/35">Suivez-nous</h4>
            <div className="flex items-center justify-center gap-3">
              <SocialLinks />
            </div>
            <div className="mt-5 text-center">
              <h5 className="mb-3 text-[11px] font-bold uppercase tracking-[0.25em] text-black/35">Langue / Devise</h5>
...
function NavColumn({ title, delay, children }: { title: string; delay: number; children: React.ReactNode }) {
  return (
    <ScrollReveal delay={delay}>
      <div>
        <h4 className="mb-6 text-[11px] font-bold uppercase tracking-[0.25em] text-black/35">{title}</h4>
...
function SocialLinks() {
  return (
    <>
      <a href="https://www.instagram.com/kayvilaconciergerie" target="_blank" rel="noopener noreferrer" aria-label="Instagram Kayvila" className="inline-flex h-11 w-11 items-center justify-center border border-black/10 text-navy/50 transition-colors hover:border-black hover:text-black">
      ...
      <a href="https://wa.me/596696681869" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp Kayvila" className="inline-flex h-11 w-11 items-center justify-center border border-black/10 text-navy/50 transition-colors hover:border-black hover:text-black">
      ...
      <a href="https://www.facebook.com/share/1EttHdFGZp/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" aria-label="Facebook Kayvila" className="inline-flex h-11 w-11 items-center justify-center border border-black/10 text-navy/50 transition-colors hover:border-black hover:text-black">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
      </a>
      <a href="https://www.tiktok.com/@kayvilaconciergerie" target="_blank" rel="noopener noreferrer" aria-label="TikTok Kayvila" className="inline-flex h-11 w-11 items-center justify-center border border-black/10 text-navy/50 transition-colors hover:border-black hover:text-black">
...
function LocaleCurrencySelectors({
  ...
  return (
    <div className="flex gap-1.5">
      <select value={locale} onChange={(e) => setLocale(e.target.value as Locale)} className="min-h-11 rounded-none border border-black/15 bg-offwhite px-2 text-[11px] text-navy focus:outline-none focus:ring-1 focus:ring-black" aria-label="Langue">
        {SUPPORTED_LOCALES.map((l) => (<option key={l} value={l}>{l === "fr" ? "FR" : l === "en" ? "EN" : "ES"}</option>))}
      </select>
      <select value={currency} onChange={(e) => setCurrency(e.target.value as Currency)} className="min-h-11 rounded-none border border-black/15 bg-offwhite px-2 text-[11px] text-navy focus:outline-none focus:ring-1 focus:ring-black" aria-label="Devise">
