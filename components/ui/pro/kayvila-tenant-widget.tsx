    ...
    <Widget className={cn("rounded-none border border-navy/8 bg-white shadow-none", className)}>
      <Widget.Header className="flex flex-row items-start justify-between gap-4 border-b border-navy/5 px-6 py-5">
        <div className="min-w-0 space-y-1">
          <Widget.Title className="block font-display text-xl font-normal text-navy">{title}</Widget.Title>
          {description ? (
            <Widget.Description className="block text-sm text-navy/55">{description}</Widget.Description>
          ) : null}
        </div>
