...
export function EditorialImageSplit({
  ...
  const textBlock = (
    ...
    >
      {eyebrow ? (
        <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-navy/80">{eyebrow}</p>
      ) : null}
      <h2
  ...
  return (
    <section className="border-y border-navy/10 bg-offwhite px-5 py-20 sm:px-6 md:py-28">
      <div className="mx-auto max-w-5xl text-center md:text-left">
        <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-navy/80">{label}</p>
        <p className="mt-4 font-display text-6xl text-navy md:text-7xl lg:text-8xl">{figure}</p>
        <p className="mt-4 max-w-xl text-sm uppercase tracking-[0.18em] text-navy/80 md:mx-0">{caption}</p>
        {detailsCaption ? (
