...
export default function DocumentsPage() {
  ...
  const printInvoice = (b: BookingDoc) => {
    ...
    if (!w) return;
    w.document.write(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Facture Kayvila</title><style>body{font-family:Georgia,serif;max-width:600px;margin:60px auto;padding:20px;color:#0A0A0A}h1{font-size:24px;margin-bottom:4px}.gold{color:#D4AF37}.line{height:1px;background:#D4AF37;margin:20px 0}.footer{margin-top:40px;font-size:11px;color:#8B8B8B;line-height:1.6}</style></head><body><h1>Kayvila</h1><p class="gold">Conciergerie de standing — Martinique</p><div class="line"></div><p><strong>Séjour :</strong> ${b.villa_name}</p><p><strong>Dates :</strong> ${fmt(b.start_date)} → ${fmt(b.end_date)}</p><div class="line"></div><div class="footer"><p>Kayvila Conciergerie</p><p>contact@kayvila.com — +596 696 00 00 00</p><p>Facture générée le ${new Date().toLocaleDateString("fr-FR")}</p></div></body></html>`
    );
    w.document.close();
